...
class TestAPIYahoo(unittest.TestCase):
    ...
    def test_tickers_list_fields_none(self, mock_yf_download):
        ...
        df_data = [[13.14,
         ...
         235.22,
         148949700.0]]
        df_columns = pd.MultiIndex.from_product([tickers,fields], names=['Ticker', 'Price'])
        df_index = pd.DatetimeIndex([data_date], name='Date', freq=None)
        mock_yf_download.return_value = pd.DataFrame(df_data, columns=df_columns, index=df_index)
        
        
        actual_output = ydh(tickers, start_date=data_date, end_date=data_date+pd.Timedelta(days=1))
        expected_output = pd.DataFrame(df_data, columns=df_columns, index=df_index)
        call_args_expected = {'tickers': tickers,
                              'start': data_date,
    ...
    def test_tickers_list_fields_list_of_2(self, mock_yf_download):
        ...
        actual_output = ydh(tickers, fields=fields_wanted, start_date=data_date, end_date=data_date+pd.Timedelta(days=1))
        expected_output = pd.DataFrame(df_data, columns=df_columns, index=df_index)
    ...
    def test_tickers_list_fields_str(self, mock_yf_download):
        ...
        expected_output = pd.DataFrame(df_data, columns=df_columns, index=df_index)
        column_mask = expected_output.columns.get_level_values(1).isin([fields_wanted])
        expected_output = expected_output.iloc[:, column_mask].T.reset_index(level=1, drop=True).T
    ...
    def test_fields_type_error(self, mock_yf_download):
        ...
        with self.assertRaises(TypeError) as context:
            ydh(tickers, fields=fields_wanted, start_date=data_date, end_date=data_date+pd.Timedelta(days=1))
        self.assertEqual(context.exception.args[0], f"Unexpected fields type: {fields_wanted}")
